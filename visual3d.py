...
class Visual3D:
    ...
    def __init__(self, radii, velocities, semi_major_axis, inclination=0.0):
        ...
        pygame.display.set_caption("3D Orbital Simulation")
        ...
        self.time_scale = 800000.0  # Speed up factor (1 million times real time)
    ...
    def get_interpolated_values(self, position):
        ...
        This function allows smooth animation between discrete data points.
        Instead of jumping from point to point, we calculate intermediate values.

        For example, if position = 45.7:
        - index_low = 45 (the point before)
        - index_high = 46 (the point after)
        - t = 0.7 (70% of the way between them)
        - We blend the values: 30% of point 45 + 70% of point 46
        ...
        t = position - index_low  # Fractional part (0.0 to 1.0)

        # Linear interpolation formula: value = start * (1-t) + end * t
    ...
    def calculate_distance_between_points(self, index1, index2):
        ...
        This is crucial for physics-based animation. We need to know the actual
        distance between consecutive points in the orbit to convert velocity
        (meters/second) into position increment (array indices/frame).

        Steps:
        1. Get the 3D positions of both points
        2. Calculate Euclidean distance: sqrt((x2-x1)² + (y2-y1)² + (z2-z1)²)
        3. Return a minimum value to prevent division by zero errors

        Without this, can't accurately simulate orbital motion at real velocities.
        """
        if index1 == index2:
        ...
        # 3D distance formula (Pythagorean theorem in 3D)
    ...
    def get_angle_for_position(self, position):
        ...
        The radii array only contains half the orbit (perihelion to aphelion).
        We traverse it twice to make a complete orbit:
        - Forward (0 to max): angle goes 0° to 180° (first half of ellipse)
        - Backward (max to 0): angle goes 180° to 360° (second half of ellipse)

        This creates a continuous orbit from the same radius data.
        The formula ensures smooth transition at perihelion and aphelion.
        """
        ...
        # Normalize position to 0-1 range within the current half
        normalized = position / max_pos

        if self.going_forward:
            # First half of orbit: 0 to π (perihelion to aphelion)
            angle = normalized * math.pi
        else:
            # Second half of orbit: π to 2π (aphelion back to perihelion)
            # When position decreases from max to 0, angle increases from π to 2π
            # Formula: angle = π * (2 - normalized)
            # At max_pos: normalized=1, angle=π (180°)
            # At 0: normalized=0, angle=2π (360°/0°)
    ...
    def get_3d_position(self, radius, angle):
        ...
        This is where the orbital inclination is applied using 3D rotation.

        Process:
        1. Start with 2D ellipse in XY plane using polar coordinates:
           x = radius * cos(angle)
           y = radius * sin(angle)

        2. Apply inclination by rotating around the X axis:
           - Y coordinate becomes: y * cos(inclination)
           - Z coordinate becomes: y * sin(inclination)

        This tilts the orbit out of the flat plane. Without inclination,
        all orbits would be flat (z=0). With inclination, the orbit appears
        tilted when viewed from different angles.

        The scale factor converts from meters to normalized OpenGL units.
        """
        # Position in flat orbital plane (XY)
        x = radius * math.cos(angle) * self.scale
        y_orbit = radius * math.sin(angle) * self.scale

        # Apply inclination (rotate around X axis)
        # This is a 3D rotation matrix applied to the Y coordinate
    ...
    def setup_camera(self):
        """Setup camera view using OpenGL transformations

        OpenGL uses a transformation stack to position the camera.
        Transformations are applied in REVERSE order (last to first):

        1. glRotatef(Y) - Rotate view left/right around Y axis
        2. glRotatef(X) - Rotate view up/down around X axis
        3. glTranslatef(z) - Move camera back from origin
        4. glTranslatef(x,y) - Pan camera left/right/up/down

        Think of it like: first we pan, then we move back, then we rotate
        to look at the scene from different angles.
        """
        glLoadIdentity()
    ...
    def run(self):
        ...
        while running:
            ...
            # dt = time elapsed per frame in simulation time
            # Real time: 1/60 second per frame (60 FPS)
            # Simulated time: (1/60) * 1,000,000 = ~16,667 simulated seconds per frame
            dt = (1.0 / fps) * self.time_scale

            # Get current velocity (meters per second from vis-viva equation)
            ...
            # Example: 30,000 m/s × 16,667 s = 500 million meters per frame
            distance_to_travel = current_velocity * dt

            # Calculate distance between current and next array position
            # This tells us how many meters one "position increment" represents
            ...
            # segment_distance is in scaled OpenGL units, distance_to_travel is in meters
            # We need to scale distance_to_travel by self.scale to match units
            # Then: increment = (distance we want to travel) / (distance per array step)
            # This gives us how many array positions to move
            ...
            # Without this limit, numerical errors could cause wild jumps
            ...
            # Update position
            # This is the "ping-pong" animation pattern that creates a complete orbit
            # from half the data (perihelion to aphelion and back)
            if self.going_forward:
                self.current_position += position_increment
                if self.current_position >= len(self.radii) - 1:
                    # Reached aphelion (farthest point), reverse direction
                ...
                if self.current_position <= 0:
                    # Reached perihelion (closest point), reverse direction
